...
fn int_test_bpmap() {
    ...
    run_test(|| {
        ...
        let mut map: BPMap<u32, u32> = BPMap::with_degrees(&format!("{}.dat", test_name), 5, 5).expect("Could not create B+ tree.");
        ...
        map = BPMap::open(&format!("{}.dat", test_name)).expect("Could not open B+ tree.");
